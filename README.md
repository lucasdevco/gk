...
The bundled exporter uses OTLP over gRPC. View traces in Jaeger at <http://localhost:16686>, metrics in Prometheus at <http://localhost:9090>, and Grafana at <http://localhost:3000>. Local Grafana allows anonymous organization administration without login and listens only on 127.0.0.1. Grafana automatically provisions Prometheus and Jaeger data sources and the GK Order Checkout Demo dashboard. Logs remain on stdout; they are not exported through OTLP.

In development, [API docs](http://localhost:8080/api/docs) can run an order checkout: real PostgreSQL order creation → stock reservation → local HTTP payment → confirmation. Use `normal`, `slow-payment` (slow payment), `payment-retry`, `out-of-stock` (409), or `payment-declined` (422), with `{}` or `{"quantity":2}`. Each run uses isolated demo fixtures and rolls back, including successful orders; the payment service never charges money. This transaction layout is for isolated demonstrations, not a production payment architecture. View the [checkout dashboard](http://localhost:3000/d/gk-observability) and use `traceId` to inspect the full chain in Jaeger. `python3 scripts/traffic.py --demo` generates these order flows.
...
Generate traffic after starting the API and enabling telemetry: `python3 scripts/traffic.py --duration 180 --rate 10`. The script sends real requests through normal → 4xx errors → recovery phases without creating or modifying task data. Use `--url` for another origin, `--concurrency` to bound in-flight requests, or `--demo` to switch to the isolated order checkout scenarios. With default 60-second exports, use `--duration 600` so each phase spans multiple exports. The default mode does not fabricate 5xx; real server failures such as database outages raise that ratio.
